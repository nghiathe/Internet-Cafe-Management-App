...
namespace QLquannet
{
    public partial class frmIntakeFood : Form
    {
        ...
        private void Filter(int? categoryId, byte? foodId = null, string search = "")
        {
            try
            {
                CurrencyManager currencyManager = (CurrencyManager)BindingContext[dgvFood.DataSource];
                ...
                foreach (DataGridViewRow row in dgvFood.Rows)
                {
                    if (!row.IsNewRow)
                    {
                        ...
                        {
                            if ((int)row.Cells["CategoryID"].Value != categoryId.Value)
                            {
                                isVisible = false;
                            }
                        }
                        ...
                        if (foodId.HasValue)
                        {
                            if ((byte)row.Cells["FoodID"].Value != foodId.Value)
                            {
                                isVisible = false;
                            }
                        }
                        ...
                        {
                            if (!row.Cells["FoodName"].Value.ToString().ToLower().Contains(search.ToLower()))
                            {
                                isVisible = false;
                            }
                        }
            ...
            {
                // Xử lý lỗi một cách thích hợp (ghi log, hiển thị thông báo, v.v.)
                MessageBox.Show($"Lỗi mất rồi, thử reset bộ lọc trước xem: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
